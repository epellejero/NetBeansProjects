...
FROM
	pago
WHERE 
	DATE_FORMAT(fecha_pago, "%Y %m %d") > "2007-01-01"
AND 
	DATE_FORMAT(fecha_pago, "%Y %m %d") < "2008-01-01"	
GROUP BY 
	codigo_cliente;
...
FROM
	pago
WHERE 
	fecha_pago BETWEEN "2008-01-01" AND "2009-01-01"
GROUP BY 
	codigo_cliente;
...
WHERE
	estado = 'Entregado'
AND  
	DATE_ADD(fecha_esperada, INTERVAL 2 DAY);
...
WHERE
	estado = 'Entregado'
AND  
	DATEDIFF(p.fecha_entrega,p.fecha_esperada) > 1;
...
JOIN producto ON d.codigo_producto = producto.codigo_producto
JOIN gama_producto g ON producto.gama = g.gama;
/*GROUP BY g.gama;*/
...
FROM
	cliente c
WHERE NOT EXISTS (SELECT p.codigo_cliente FROM pedido p WHERE p.codigo_cliente = c.codigo_cliente);    
...
FROM
	cliente c
WHERE NOT EXISTS (SELECT pa.codigo_cliente FROM pago pa WHERE pa.codigo_cliente = c.codigo_cliente)
AND NOT EXISTS (SELECT pe.codigo_cliente FROM pedido pe WHERE pe.codigo_cliente = c.codigo_cliente);    
...
SELECT COUNT(codigo_pedido) AS Cantidad, estado FROM pedido GROUP BY estado ORDER BY Cantidad;

/* 5. Calcula el precio de venta del producto más caro y más barato en una misma consulta. */

SELECT MAX(precio_venta) AS Maxino, MIN(precio_venta) AS Minimo FROM producto; 

/* 6. Calcula el número de clientes que tiene la empresa.*/

SELECT COUNT(codigo_cliente) AS Cantidad FROM cliente;

/* 7. ¿Cuántos clientes tiene la ciudad de Madrid? */

SELECT COUNT(codigo_cliente) AS Cantidad FROM cliente WHERE ciudad='Madrid';

/* 8. ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?*/

SELECT ciudad, COUNT(codigo_cliente) AS Cantidad FROM cliente WHERE ciudad LIKE 'M%' GROUP BY ciudad;

/* 9. Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende cada uno.52*/
...
SELECT
	codigo_producto,
	COUNT(codigo_producto) AS Cantidad
FROM
	detalle_pedido 
GROUP BY codigo_producto ORDER BY Cantidad DESC LIMIT 20;
...
SELECT 
	(SELECT SUM(pr.precio_proveedor * dp.cantidad) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS BI,
    (SELECT SUM(pr.precio_proveedor * dp.cantidad*.21) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS IVA,
    (SELECT SUM(pr.precio_proveedor * dp.cantidad*1.21) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS Total
FROM 
	detalle_pedido dp
limit 1;
...
SELECT
	dp.codigo_producto,
	(SELECT (pr.precio_proveedor * dp.cantidad) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS BI,
    (SELECT (pr.precio_proveedor * dp.cantidad*.21) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS IVA,
    (SELECT (pr.precio_proveedor * dp.cantidad*1.21) 
    FROM detalle_pedido dp JOIN producto pr ON dp.codigo_producto = pr.codigo_producto) AS Total
FROM 
	detalle_pedido dp
